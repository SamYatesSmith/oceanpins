...
def get_divelog_markers(request):
    ...
    markers = [
        {
            "lat": float(dive['location'].split(',')[0]),
            "lng": float(dive['location'].split(',')[1])
        }
        for dive in divelogs if dive['location']
    ]
...
def interactive_map(request):
    ...
    dive_logs = DiveLog.objects.filter(user=user).values(
        'id', 'date', 'name', 'location', 'buddy', 'depth',
        'temp', 'visibility', 'bottom_time', 'user_id'
    )
...
def update_dive_log(request):
    try:
            ...
            return JsonResponse(
                {
                    'status': 'warning',
                    'message': (
                        'Invalid data received: Missing ID, but proceeding with default handling.'
                    )
                },
                status=200
            )
        ...
        return JsonResponse(
            {'status': 'error', 'message': 'Dive log not found'},
            status=404
        )
    except Exception as e:
    ...
    try:
        ...
        if not dive_log_id:
            return JsonResponse(
                    {
                        'status': 'success',
                        'message': 'Marker without ID removed successfully'
                    }
                )
